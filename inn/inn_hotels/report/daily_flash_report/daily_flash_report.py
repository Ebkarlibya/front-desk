...
def get_total_room():
    return frappe.db.sql(
        """
        SELECT 
            SUM(CASE WHEN rt.name = %s THEN 1 ELSE 0 END) AS studio,
            SUM(CASE WHEN rt.name = %s THEN 1 ELSE 0 END) AS superior,
            SUM(CASE WHEN rt.name = %s THEN 1 ELSE 0 END) AS deluxe,
            SUM(CASE WHEN rt.name = %s THEN 1 ELSE 0 END) AS executive,
            SUM(CASE WHEN rt.name = %s THEN 1 ELSE 0 END) AS suite,
            COUNT(*) AS total
        FROM `tabInn Room` r
        LEFT JOIN `tabInn Room Type` rt ON r.room_type=rt.name""",
        (
            get_setting_value("studio"),
            get_setting_value("superior"),
            get_setting_value("deluxe"),
            get_setting_value("executive"),
            get_setting_value("suite"),
        ),
        as_dict=True,
    )
...
def get_folio_transaction(current_year, next_year):
    return frappe.db.sql(
        ...
        where flag='Credit' and audit_date>=%s and audit_date<%s""",
        (current_year, next_year),
        as_dict=True,
    )
...
def get_setting_value(field_name):
    setting = frappe.get_doc("Inn Hotels Setting")
    return setting.get(field_name)
    ...
    if filters.date:
        ...
        for rb in room_booking:
            ...
            for i in range((end - start).days):
                ...
                elif availability == "Room Sold" and (
                    rb.status == "Stayed" or rb.status == "Finished"
                ):
        ...
        room["Total Saleable Room"] = {
            "today_actual": room["Total Room Available"]["today_actual"]
            - room["Total Room Out of Order"]["today_actual"],
            "mtd_actual": room["Total Room Available"]["mtd_actual"]
            - room["Total Room Out of Order"]["mtd_actual"],
            "mtd_last_month": room["Total Room Available"]["mtd_last_month"]
            - room["Total Room Out of Order"]["mtd_last_month"],
            "year_to_date": room["Total Room Available"]["year_to_date"]
            - room["Total Room Out of Order"]["year_to_date"],
        }
        ...
        for r in reservation:
            ...
            if start == end:
                room["Total Day Use"]["year_to_date"] = (
                    room["Total Day Use"]["year_to_date"] + 1
                )
                if start == today:
                    room["Total Day Use"]["today_actual"] = (
                        room["Total Day Use"]["today_actual"] + 1
                    )
                if start >= current_month and start <= today:
                    room["Total Day Use"]["mtd_actual"] = (
                        room["Total Day Use"]["mtd_actual"] + 1
                    )
                elif start >= last_month and start < current_month:
                    room["Total Day Use"]["mtd_last_month"] = (
                        room["Total Day Use"]["mtd_last_month"] + 1
                    )
            ...
            for i in range((end - start).days):
                ...
                if status == "In House":
                    room["Total In House"]["year_to_date"] = (
                        room["Total In House"]["year_to_date"] + 1
                    )
                    average_room_rate["year_to_date"] = (
                        average_room_rate["year_to_date"] + r["actual_room_rate"]
                    ) / 2
                    if date == today:
                        room["Total In House"]["today_actual"] = (
                            room["Total In House"]["today_actual"] + 1
                        )
                        average_room_rate["today_actual"] = (
                            average_room_rate["today_actual"] + r["actual_room_rate"]
                        ) / 2
                    if date >= current_month and date <= today:
                        room["Total In House"]["mtd_actual"] = (
                            room["Total In House"]["mtd_actual"] + 1
                        )
                        average_room_rate["mtd_actual"] = (
                            average_room_rate["mtd_actual"] + r["actual_room_rate"]
                        ) / 2
                    elif date >= last_month and date < current_month:
                        room["Total In House"]["mtd_last_month"] = (
                            room["Total In House"]["mtd_last_month"] + 1
                        )
                        average_room_rate["mtd_last_month"] = (
                            average_room_rate["mtd_last_month"] + r["actual_room_rate"]
                        ) / 2

                    channel = r.channel
                    if channel == "Walk In":
                        room["Total Walk In"]["year_to_date"] = (
                            room["Total Walk In"]["year_to_date"] + 1
                        )
                        if date == today:
                            room["Total Walk In"]["today_actual"] = (
                                room["Total Walk In"]["today_actual"] + 1
                            )
                        if date >= current_month and date <= today:
                            room["Total Walk In"]["mtd_actual"] = (
                                room["Total Walk In"]["mtd_actual"] + 1
                            )
                        elif date >= last_month and date < current_month:
                            room["Total Walk In"]["mtd_last_month"] = (
                                room["Total Walk In"]["mtd_last_month"] + 1
                            )
        ...
        for ge in gl_entry:
            ...
            if (
                ...
                revenue[account]["year_to_date"] = (
                    revenue[account]["year_to_date"] + ge.credit - ge.debit
                )
                if ge.posting_date == today:
                    revenue[account]["today_actual"] = (
                        revenue[account]["today_actual"] + ge.credit - ge.debit
                    )
                if ge.posting_date >= current_month and ge.posting_date <= today:
                    revenue[account]["mtd_actual"] = (
                        revenue[account]["mtd_actual"] + ge.credit - ge.debit
                    )
                elif ge.posting_date >= last_month and ge.posting_date < current_month:
                    revenue[account]["mtd_last_month"] = (
                        revenue[account]["mtd_last_month"] + ge.credit - ge.debit
                    )
        ...
        for ft in folio_transaction:
            payment[ft.mode_of_payment]["year_to_date"] = (
                payment[ft.mode_of_payment]["year_to_date"] + ft.amount
            )
            if ft.audit_date == today:
                payment[ft.mode_of_payment]["today_actual"] = (
                    payment[ft.mode_of_payment]["today_actual"] + ft.amount
                )
            if ft.audit_date >= current_month and ft.audit_date <= today:
                payment[ft.mode_of_payment]["mtd_actual"] = (
                    payment[ft.mode_of_payment]["mtd_actual"] + ft.amount
                )
            elif ft.audit_date >= last_month and ft.audit_date < current_month:
                payment[ft.mode_of_payment]["mtd_last_month"] = (
                    payment[ft.mode_of_payment]["mtd_last_month"] + ft.amount
                )
        ...
        data.append(
            {
                ...
                "today_actual": "{:,}".format(
                    int(round(average_room_rate["today_actual"]))
                ).replace(",", "."),
                "mtd_actual": "{:,}".format(
                    int(round(average_room_rate["mtd_actual"]))
                ).replace(",", "."),
                "mtd_last_month": "{:,}".format(
                    int(round(average_room_rate["mtd_last_month"]))
                ).replace(",", "."),
                "year_to_date": "{:,}".format(
                    int(round(average_room_rate["year_to_date"]))
                ).replace(",", "."),
                "indent": 0.0,
                "is_currency": True,
            }
        ...
        for key in revenue:
                ...
                data.append(
                    {
                        ...
                        "today_actual": "{:,}".format(
                            int(round(total_revenue["today_actual"]))
                        ).replace(",", "."),
                        "mtd_actual": "{:,}".format(
                            int(round(total_revenue["mtd_actual"]))
                        ).replace(",", "."),
                        "mtd_last_month": "{:,}".format(
                            int(round(total_revenue["mtd_last_month"]))
                        ).replace(",", "."),
                        "year_to_date": "{:,}".format(
                            int(round(total_revenue["year_to_date"]))
                        ).replace(",", "."),
                        "indent": 0.0,
                        "is_currency": True,
                    }
            ...
            data.append(
                {
                    "statistic": title,
                    "today_actual": "{:,}".format(
                        int(round(revenue[key]["today_actual"]))
                    ).replace(",", "."),
                    "mtd_actual": "{:,}".format(
                        int(round(revenue[key]["mtd_actual"]))
                    ).replace(",", "."),
                    "mtd_last_month": "{:,}".format(
                        int(round(revenue[key]["mtd_last_month"]))
                    ).replace(",", "."),
                    "year_to_date": "{:,}".format(
                        int(round(revenue[key]["year_to_date"]))
                    ).replace(",", "."),
                    "indent": indent,
                    "is_currency": True,
                }
            ...
            if key != "2141.000" and key != "2110.004":
                total_revenue = {
                    "today_actual": total_revenue["today_actual"]
                    + revenue[key]["today_actual"],
                    "mtd_actual": total_revenue["mtd_actual"]
                    + revenue[key]["mtd_actual"],
                    "mtd_last_month": total_revenue["mtd_last_month"]
                    + revenue[key]["mtd_last_month"],
                    "year_to_date": total_revenue["year_to_date"]
                    + revenue[key]["year_to_date"],
                }
            ...
            data.append(
                {
                    "statistic": key,
                    "today_actual": "{:,}".format(
                        int(round(payment[key]["today_actual"]))
                    ).replace(",", "."),
                    "mtd_actual": "{:,}".format(
                        int(round(payment[key]["mtd_actual"]))
                    ).replace(",", "."),
                    "mtd_last_month": "{:,}".format(
                        int(round(payment[key]["mtd_last_month"]))
                    ).replace(",", "."),
                    "year_to_date": "{:,}".format(
                        int(round(payment[key]["year_to_date"]))
                    ).replace(",", "."),
                    "indent": 1.0,
                    "is_currency": True,
                }
