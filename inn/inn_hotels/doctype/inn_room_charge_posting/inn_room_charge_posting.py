...
from inn.inn_hotels.doctype.inn_folio_transaction_type.inn_folio_transaction_type import (
    get_accounts_from_id,
)
...
from inn.inn_hotels.doctype.inn_tax.inn_tax import (
    calculate_inn_tax_and_charges,
...
def is_there_closed_room_charge_posting_at():
    ...
    if frappe.db.exists(
        "Inn Room Charge Posting", {"audit_date": date, "status": "Closed"}
    ):
        return 1
    else:
        return 2
...
def post_individual_room_charges(parent_id, tobe_posted_list):
    ...
    room_post_settings = frappe.db.get_values_from_single(
        ...
        filters="",
        fields=["profit_sharing_account", "profit_sharing_transaction_type"],
        as_dict=True,
    )[0]
    ...
    channel_exclude_tax = frappe.get_list(
        "Inn Channel Tax Exclude",
    ...
    inn_settings = frappe.get_value(
        "Inn Hotels Setting",
        None,
        ["maximum_payment_exclude", "inn_tax_exclude_option"],
        as_dict=1,
    )
    ...
    for item in list_json:
        ...
        ftb_doc.transaction_type = transaction_types["room_charge"]  # Use dynamic value
        ftb_doc.insert()
        ...
        if (reservation.channel in channel_exclude_tax) or (
            reservation.actual_room_rate <= flt(maximum_price_exclude_tax)
        ):
        ...
        room_charge_folio_trx.transaction_type = transaction_types[
            "room_charge"
        ]  # Use dynamic value
        ...
        room_tb_id, room_tb_amount, _ = calculate_inn_tax_and_charges(
            reservation.nett_actual_room_rate, reservation.actual_room_rate_tax
        )
        ...
        if breakfast_price > 0:
            ...
            breakfast_charge_debit_account, breakfast_charge_credit_account = (
                get_accounts_from_id(transaction_types["breakfast_charge"])
            )
            ...
            breakfast_charge_folio_trx.transaction_type = transaction_types[
                "breakfast_charge"
            ]
            ...
            breakfast_charge_folio_trx.remark = (
                "Room Charge: Breakfast (Nett): "
                + item_doc.room_id
                + " - "
                + get_last_audit_date().strftime("%d-%m-%Y")
            )
...
def post_room_charges(parent_id, tobe_posted_list):
    ...
    for item in list_json:
        # Create Inn Folio Transaction Bundle
        ftb_doc = frappe.new_doc("Inn Folio Transaction Bundle")
        ftb_doc.transaction_type = transaction_types["room_charge"]
        ftb_doc.insert()
        ...
        room_charge_folio_trx.transaction_type = transaction_types["room_charge"]
        room_charge_folio_trx.amount = float(int(reservation.nett_actual_room_rate))
        accumulated_amount += float(int(reservation.nett_actual_room_rate))
        room_charge_folio_trx.debit_account = room_charge_debit_account
        room_charge_folio_trx.credit_account = room_charge_credit_account
...
def calculate_already_posted_total(room_charge_posting_id):
    ...
    frappe.db.set_value(
        "Inn Room Charge Posting", doc.name, "already_posted_total", total
    )


def recalculate_nett_charge(reservation_doc):
    room_tax_name, new_room_charge, breakfast_tax_name, new_breakfast_charge = (
        recalculate_tax_by_exclude_tax(reservation_doc)
    )
...
def __get_actual_room_rate_breakdown_check_commission(reservation_doc):
    ...
    channel = check_channel_commission(
        reservation_doc, reservation_price=reservation_doc.actual_room_rate
    )
    if channel.profit_sharing == PROFIT_SHARING_ENABLED:
        if channel.sharing_type != PROFIT_SHARING_TYPE_PERCENTAGE:
            raise NotImplementedError(
                "comission type other than percentage is not supported yet"
            )
    ...
    _, _, room_price = calculate_inn_tax_and_charges_exclude_commision(
        reservation_doc.actual_room_rate - room_rate_doc.final_breakfast_rate_amount,
        reservation_doc.actual_room_rate_tax,
        channel.room_cashback,
    )
    _, _, breakfast_price = calculate_inn_tax_and_charges_exclude_commision(
        room_rate_doc.final_breakfast_rate_amount,
        reservation_doc.actual_breakfast_rate_tax,
        channel.breakfast_cashback,
    )
    # diff on top
