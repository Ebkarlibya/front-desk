...
def copy_amenities_template(amenities_type_id):
    amenities_list = frappe.get_all(
        "Inn Amenities", filters={"parent": amenities_type_id}, fields=["*"]
    )
...
def update_room_status(rooms, mode):
    ...
    if mode == "clean":
        for room in json.loads(rooms):
            ...
            if door_status == "No Status" or door_status == "Sleeping Out":
                if room_status == "Vacant Dirty":
                    ...
                    frappe.db.set_value(
                        "Inn Room", room, "room_status", "Occupied Clean"
                    )
                elif room_status == "Vacant Clean" and (
                    is_housekeeping_supervisor
                    or is_housekeeping_assistant
                    or is_administrator
                ):
        ...
        if len(is_failed) > 0:
            return (
                "Some Rooms status updated. Some room status cannot be updated: "
                + str(is_failed)
            )
        else:
...
def update_single_room_status(room, mode):
    ...
    if mode == "clean":
        ...
        try:
            last_out = frappe.get_last_doc(
                doctype="Inn Room Booking",
                filters={"room_id": room},
        ...
        if last_out == None or last_out.status in ["Finished", "Canceled"]:
            ...
            room_booking.note = (
                "Set out of order by mobile from user: " + frappe.session.user
            )
