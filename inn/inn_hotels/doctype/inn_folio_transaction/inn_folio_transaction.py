...
from inn.inn_hotels.doctype.inn_folio_transaction_bundle.inn_folio_transaction_bundle import (
    get_trx_list,
)
...
def get_mode_of_payment_account(
    mode_of_payment_id, company_name=frappe.get_doc("Global Defaults").default_company
):
    return frappe.db.get_value(
        "Mode of Payment Account",
        {"parent": mode_of_payment_id, "company": company_name},
        "default_account",
    )
...
def add_package_charge(package_name, sub_folio, remark, parent):
    ...
    tb_id, tb_amount, _ = calculate_inn_tax_and_charges(
        package_doc.total_amount, package_doc.inn_tax_id
    )
    for index, package_tax_item_name in enumerate(tb_id):
        ...
        new_tax_doc.credit_account = frappe.get_doc(
            "Inn Tax Breakdown", package_tax_item_name
        ).breakdown_account
    ...
    difference = package_doc.total_amount_after_tax - (
        total_tb + package_doc.total_amount
    )
    ...
    if difference > 0:
        ...
        tax_list = frappe.get_all(
            "Inn Folio Transaction",
            filters={"ftb_id": ftb_doc.name, "transaction_type": "Package Tax"},
            fields=["*"],
            order_by="name desc",
        )
...
def void_transaction(
    ...
    requester,
    bundle_len,
    supervisor_passcode=None,
):
    list = get_trx_list(trx_id)
    print(list)
    if int(use_passcode) == 1:
        if supervisor_passcode != frappe.db.get_single_value(
            "Inn Hotels Setting", "supervisor_passcode"
        ):
            frappe.msgprint(
                frappe._(
                    "<b>Error: Passcode not correct.</b> <br /> Please consult the correct passcode to supervisor, "
...
def void_single_trx(trx_id, applicant_reason, requester):
    ...
    if void_doc.status == "Approved":
        ...
        if trx_doc.transaction_type == "Room Charge":
            already_posted_doc = frappe.get_doc(
                "Inn Room Charge Posted", {"folio_transaction_id": trx_doc.name}
            )
            rcp_doc = frappe.get_doc(
                "Inn Room Charge Posting", already_posted_doc.parent
            )
            frappe.delete_doc("Inn Room Charge Posted", already_posted_doc.name)

            posted = frappe.get_all(
                "Inn Room Charge Posted", filters={"parent": rcp_doc}, fields=["*"]
            )
            total = 0.0
            if len(posted) > 0:
                for item in posted:
            ...
            frappe.db.set_value(
                "Inn Room Charge Posting", rcp_doc.name, "already_posted_total", total
            )
