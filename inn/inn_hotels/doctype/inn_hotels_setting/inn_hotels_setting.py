...
def generate_folio_transaction_type():
    ...
    create_account(
        "Other A/R", "1130.000", "1133.000", 1, "IDR", "", "Asset", default_company
    )
    create_account(
        "A/R Guest Ledger",
        "1133.000",
    ...
    create_account(
        "Room Service Beverages Revenue",
        "4140.000",
        "4140.002",
        0,
        "IDR",
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Room Charge Tax/Service"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Breakfast Charge Tax/Service"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Credit Card Administration Fee"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
                ...
                "module": 0,
                "is_included": 0,
                "credit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "2110.013"}
                )[0].name,
                "debit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "1133.002"}
                )[0].name,
            }
        ]
        ...
        folio_transaction_type_records += [
            {
                ...
                "is_included": 0,
                "credit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "4210.001"}
                )[0].name,
                "debit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "1133.003"}
                )[0].name,
            }
        ]
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Breakfast Charge"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
        ...
        folio_transaction_type_records += [
            {
                ...
                "is_included": 0,
                "credit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "1113.000"}
                )[0].name,
                "debit_account": frappe.db.get_list(
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Additional Charge"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Restaurant Food"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
                ...
                "is_included": 1,
                "credit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "4120.001"}
                )[0].name,
                "debit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "1133.003"}
                )[0].name,
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Restaurant Beverages"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
                ...
                "is_included": 1,
                "credit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "4120.002"}
                )[0].name,
                "debit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "1133.003"}
                )[0].name,
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Restaurant Other"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
                ...
                "is_included": 1,
                "credit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "4160.000"}
                )[0].name,
                "debit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "1133.003"}
                )[0].name,
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Room Service Food"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
                ...
                "is_included": 1,
                "credit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "4140.001"}
                )[0].name,
                "debit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "1133.003"}
                )[0].name,
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Room Service Beverage"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
                ...
                "is_included": 1,
                "credit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "4140.002"}
                )[0].name,
                "debit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "1133.003"}
                )[0].name,
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "FBS -- Service 10 %"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
                ...
                "is_included": 1,
                "credit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "2110.004"}
                )[0].name,
                "debit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "1133.003"}
                )[0].name,
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "FBS -- Tax 11 %"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
                ...
                "is_included": 1,
                "credit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "2141.000"}
                )[0].name,
                "debit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "1133.003"}
                )[0].name,
        ...
        folio_transaction_type_records += [
            {
                ...
                "is_included": 0,
                "credit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "4300.001"}
                )[0].name,
                "debit_account": frappe.db.get_list(
                    "Account", filters={"account_number": "1133.003"}
                )[0].name,
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Cancellation Fee"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Late Checkout"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
    ...
    if not frappe.db.exists(
        "Inn Folio Transaction Type", {"trx_name": "Early Checkin"}
    ):
        folio_transaction_type_records += [
            {
                "doctype": "Inn Folio Transaction Type",
...
def generate_bed_type(bed_type):
    ...
    for bed in bed_type:
        ...
        if not frappe.db.exists("Inn Bed Type", bed.get("name1")):
            ...
            frappe.msgprint(
                _("Bed Type '{0}' already exists. Skipping.").format(bed.get("name1"))
            )
...
def generate_room_type(room_type):
    ...
    for bed in room_type:
        ...
        if not frappe.db.exists("Inn Room Type", bed.get("name1")):
            ...
            frappe.msgprint(
                _("Room Type '{0}' already exists. Skipping.").format(bed.get("name1"))
            )
...
def insert_role(role):
    ...
    for bed in role:
        ...
        if not frappe.db.exists("Role", bed.get("name1")):
            ...
            frappe.msgprint(
                _("Role '{0}' already exists. Skipping.").format(bed.get("name1"))
            )
...
def create_account(
    ...
    existing_account = frappe.db.get_list(
        "Account", filters={"account_number": account_number}, fields=["name"]
    )
    if existing_account:
        ...
        parent_account = None
        if parent_number:
            parent_account_query = frappe.db.get_list(
                "Account", filters={"account_number": parent_number}, fields=["name"]
            )
            if not parent_account_query:
                frappe.throw(
                    _("Parent account with number {0} not found.").format(parent_number)
                )
    ...
    for item in inn_account_setting:
        try:
            ...
            for key in required_keys:
                if key not in item:
                    frappe.throw(
                        _("Missing required key: {0} in account setting").format(key)
                    )
            ...
            frappe.msgprint(
                _("Failed to create account: {0}. Error: {1}").format(
                    item.get("account_name", "Unknown"), str(e)
                )
            )
...
def generate_supervisor_passcode():
    ...
    frappe.db.set_value(
        "Inn Hotels Setting", "Inn Hotels Setting", "supervisor_passcode", passcode
    )
...
def show_supervisor_passcode():
    if frappe.db.get_single_value("Inn Hotels Setting", "supervisor_passcode"):
        frappe.msgprint(
            frappe.db.get_single_value("Inn Hotels Setting", "supervisor_passcode")
        )
    else:
