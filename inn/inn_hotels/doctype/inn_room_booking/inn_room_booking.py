...
class InnRoomBooking(Document):
    ...
    def on_update(self):
        if frappe.db.exists(
            "Inn Guest Booking Room", {"inn_room_booking": self.name}, cache=True
        ):
            gbr = frappe.get_doc(
                "Inn Guest Booking Room", {"inn_room_booking": self.name}, cache=True
            )
...
def update_by_reservation(reservation_id):
    ...
    if reservation_doc.status == "Cancel":
        ...
        room_booking_doc = frappe.get_doc(
            "Inn Room Booking",
        ...
        return_message = (
            room_booking_doc.name
            + "Status is Canceled by Cancelling Reservation: "
            + reservation_id
        )
        ...
        return_message = (
            room_booking_doc.name
            + " Status is Finished by Checking Out Reservation: "
            + reservation_id
        )
        ...
        return_message = (
            room_booking_doc.name
            + " Updated by Checking In Process in Reservation: "
        ...
        return_message = (
            room_booking_doc.name
            + "Status is Canceled by No Show of Reservation: "
            + reservation_id
        )
    else:
        ...
        if not frappe.db.exists(
            ...
            if reservation_changed:
                ...
                return_message = (
                    room_booking_doc.name
                    + " Updated by changes in Reservation: "
                    + reservation_doc.name
                )
    ...
    if filters.get("phase") == "Check In":
        ...
        if filters.get("start") and filters.get("end"):
            if filters.get("room_type"):
                ...
                room_list = list(
                    frappe.db.sql("select name, room_type, bed_type from `tabInn Room`")
                )
...
def get_all_room_with_room_booking_status():
    ...
    room_booking_list = frappe.get_all(
        "Inn Room Booking",
        filters={"status": ["in", ["Booked", "Stayed"]]},
        fields=["*"],
    )
