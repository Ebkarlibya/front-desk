...
frappe.ui.form.on("Inn Reservation", {
  ...
  refresh: function (frm) {
    ...
    if (frm.doc.__islocal !== 1) {
      frm.add_custom_button(__("Show Folio"), function () {
        frappe.call({
          method:
            "inn.inn_hotels.doctype.inn_reservation.inn_reservation.get_folio_url",
          args: {
    ...
    if (frm.doc.__islocal !== 1 && frm.doc.status === "Reserved") {
      ...
      if (is_check_in === undefined) {
        frm.add_custom_button(__("Start Check In Process"), function () {
          ...
          frappe.call({
            method:
              "inn.inn_hotels.doctype.inn_reservation.inn_reservation.start_check_in",
            args: {
        ...
        if (
          ...
          frm.add_custom_button(__("Finish Check In Process"), function () {
            if (frm.doc.__unsaved !== undefined || frm.doc.unsaved == 1) {
              ...
              frappe.call({
                method:
                  "inn.inn_hotels.doctype.inn_reservation.inn_reservation.check_in_reservation",
                args: {
                  reservation_id: frm.doc.name,
                },
                callback: (r) => {
                  if (r.message === "In House") {
                    frappe.call({
                      method:
                        "inn.inn_hotels.doctype.inn_room_booking.inn_room_booking.update_by_reservation",
                      args: {
                        reservation_id: frm.doc.name,
                      },
                      callback: (r) => {
      ...
      frm.page.add_menu_item(__("Cancel"), function () {
        frappe.confirm(
          __("You are about to Cancel this Reservation. Are you sure?"),
          function () {
            frappe.call({
              method:
                "inn.inn_hotels.doctype.inn_reservation.inn_reservation.cancel_reservation",
              args: {
      ...
      frm.page.add_menu_item(__("No Show"), function () {
        frappe.confirm(
          __("You are about to mark the guest as No Show. Are you Sure?"),
          function () {
            frappe.call({
              method:
                "inn.inn_hotels.doctype.inn_reservation.inn_reservation.no_show_reservation",
              args: {
    ...
    if (frm.doc.__islocal !== 1 && frm.doc.status === "In House") {
      ...
      frm.page.add_menu_item(__("Cancel"), function () {
        frappe.confirm(
          __(
            "You are about to Cancel this Reservation with status already <b>In House</b>. Are you sure?"
          ),
          function () {
            frappe.call({
              method:
                "inn.inn_hotels.doctype.inn_reservation.inn_reservation.cancel_single_reservation_in_house",
              args: {
                reservation_id: frm.doc.name,
              },
              callback: (r) => {
      ...
      frm.fields_dict.arrival.$input.on("click", function (evt) {
        frappe.show_alert(
          __(
            "<b>Warning!</b> <br /> Changing Actual Arrival when Reservation status is In House may cause data inconsistencies."
          )
        );
      });
      frm.fields_dict.departure.$input.on("click", function (evt) {
        frappe.show_alert(
          __(
            "<b>Warning!</b> <br /> Changing Actual Departure when Reservation status is In House may cause data inconsistencies."
          )
        );
      });
    }
  ...
  expected_arrival: function (frm) {
    frappe.call({
      method:
        "inn.inn_hotels.doctype.inn_audit_log.inn_audit_log.get_last_audit_date",
      callback: (r) => {
        if (r.message) {
          if (frm.doc.expected_arrival < r.message) {
            ...
            if (
              frm.doc.expected_departure &&
              (frm.doc.__islocal == 1 || frm.doc.status == "Reserved")
            ) {
              frm.set_value(
                "total_night",
                calculate_nights(
                  frm.doc.expected_arrival,
                  frm.doc.expected_departure
                )
              );
            }
    ...
    frappe.call({
      ...
      callback: (r) => {
        if (r.message) {
          ...
          expected_arrival_date.setHours(
            now.getHours(),
            now.getMinutes(),
            now.getSeconds()
          );
          let default_arrival = expected_arrival_date
            .toISOString()
            .replace("T", " ");
          default_arrival = default_arrival.substring(
            0,
            default_arrival.length - 5
          );
          ...
          if (frm.doc.arrival < r.message) {
            ...
            if (frm.doc.departure) {
              frm.set_value(
                "total_night",
                calculate_nights(frm.doc.arrival, frm.doc.departure)
              );
            }
            toggle_room_detail(frm);
          }
    ...
    frappe.call({
      ...
      callback: (r) => {
        if (r.message) {
          ...
          let default_departure = default_departure_date
            .toISOString()
            .replace("T", " ");
          default_departure = default_departure.substring(
            0,
            default_departure.length - 5
          );
          ...
          if (frm.doc.departure < r.message) {
            ...
            frappe.call({
              method:
                "inn.inn_hotels.doctype.inn_room_booking.inn_room_booking.get_room_booking_name_by_reservation",
              args: {
                reservation_id: frm.doc.name,
              },
              callback: (r) => {
  ...
  room_rate: function (frm) {
    ...
    frappe.call({
      method:
        "inn.inn_hotels.doctype.inn_room_rate.inn_room_rate.get_base_room_rate",
      args: {
        ...
        frappe.call({
          ...
          callback: (r) => {
            if (r.message !== "ERROR") {
              frm.reload_doc();
              frappe.show_alert(
                __("Card " + r.message + " issued for this Reservation.")
              );
              return;
            } else {
...
function autofill(frm) {
  ...
  if (
    ...
    frm.set_value(
      "departure",
      format_date_to_string_arrival(expected_departure)
    );
  }
  if (
    frm.doc.actual_room_rate === undefined ||
    frm.doc.actual_room_rate == null ||
    parseFloat(frm.doc.actual_room_rate) == 0.0
  ) {
    frm.set_value("actual_room_rate", frm.doc.init_actual_room_rate);
  }
  if (
    frm.doc.actual_room_id === undefined ||
    frm.doc.actual_room_id == null ||
    frm.doc.actual_room_id === ""
  ) {
    frappe.call({
...
function handle_filter_on_start_if_filled(frm) {
  let start_date = formatDate(
    frm.doc.arrival ? frm.doc.arrival : frm.doc.expected_arrival
  );
  let end_date = formatDate(
    frm.doc.departure ? frm.doc.departure : frm.doc.expected_departure
  );

  let query = {
    room_id:
      "inn.inn_hotels.doctype.inn_room_booking.inn_room_booking.get_room_available",
    actual_room_id:
      "inn.inn_hotels.doctype.inn_room_booking.inn_room_booking.get_room_available",
    room_type:
      "inn.inn_hotels.doctype.inn_room_booking.inn_room_booking.get_room_type_available",
    bed_type:
...
function get_room_max_active_card() {
  frappe.call({
    method:
      "inn.inn_hotels.doctype.inn_key_card.inn_key_card.room_max_active_card",
    callback: (r) => {
      if (r.message) {
  ...
  frappe.call({
    ...
    callback: (r) => {
      if (r.message === "ERROR") {
        ...
        cur_frm.reload_doc();
        if (flag === "with") {
          frappe.show_alert(__("Card Erased."));
          return;
        } else if (flag === "without") {
          frappe.show_alert(__("Card Deactivated"));
          return;
        }
      }
    },
...
function make_read_only(frm) {
  ...
  if (
    ...
    frm.doc.status === "Finish" ||
    frm.doc.status === "No Show"
  ) {
    frm.disable_save();
    active_flag = 1;
  } else {
...
function process_check_out(frm) {
  frappe.call({
    method:
      "inn.inn_hotels.doctype.inn_folio.inn_folio.get_balance_by_reservation",
    args: {
      reservation_id: frm.doc.name,
    },
    callback: (r) => {
      if (r.message !== 0) {
        ...
        if (current_active_card > 0) {
          ...
          frappe.call({
            method:
              "inn.inn_hotels.doctype.inn_reservation.inn_reservation.check_out_reservation",
            args: {
              reservation_id: frm.doc.name,
            },
            callback: (r) => {
...
function move_room(frm) {
                ...
                return {
                  filters: [
                    [
                      ...
                      "room_type",
                      "=",
                      d.fields_dict["mv_room_type"].get_value(),
                    ],
                    ["Inn Room Rate", "is_disabled", "=", 0],
                    [
                      "Inn Room Rate",
  ...
  d.set_primary_action(__("Move Room"), () => {
    ...
    if (d.get_values().mv_change_rate === 1) {
      if (
        d.get_values().mv_room_rate === undefined ||
        d.get_values().mv_room_rate === "" ||
        d.get_values().mv_actual_room_rate === undefined ||
...
function calculate_rate_and_bill(frm) {
  frappe.call({
    method:
      "inn.inn_hotels.doctype.inn_reservation.inn_reservation.calculate_room_bill",
    args: {
...
function check_membership_cards() {
  ...
  d.set_primary_action(__("Check"), () => {
    frappe.call({
      method:
        "inn.inn_hotels.doctype.inn_membership_card.inn_membership_card.check_card",
      args: {
