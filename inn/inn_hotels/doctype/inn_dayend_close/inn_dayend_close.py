...
def process_dayend_close(doc_id):
    ...
    COMMISSION_TRANSACTION_TYPE = frappe.db.get_single_value(
        "Inn Hotels Setting", fieldname="profit_sharing_transaction_type"
    )
    ...
    if need_resolve_flag:
        ...
        # create_je_for_inn_restaurant_finished_order()
    ...
    for order in order_list:
        ...
        if restaurant_food > 0:
            ...
            food_debit_account = frappe.get_doc(
                "Inn Folio Transaction Type", "Restaurant Food"
            ).debit_account
        ...
        if restaurant_beverage > 0:
            ...
            bev_debit_account = frappe.get_doc(
                "Inn Folio Transaction Type", "Restaurant Beverages"
            ).debit_account
        ...
        if restaurant_other > 0:
            ...
            other_debit_account = frappe.get_doc(
                "Inn Folio Transaction Type", "Other Restaurant"
            ).debit_account
        ...
        if float(order.rounding_amount) > 0:
            ...
            ro_debit_account = frappe.get_doc(
                "Inn Folio Transaction Type", "Round Off"
            ).debit_account
        ...
        srv_debit_account = frappe.get_doc(
            "Inn Folio Transaction Type", "FBS -- Service 10 %"
        ).debit_account
        ...
        tax_debit_account = frappe.get_doc(
            "Inn Folio Transaction Type", "FBS -- Tax 11 %"
        ).debit_account
