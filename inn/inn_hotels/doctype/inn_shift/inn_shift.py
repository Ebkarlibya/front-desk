...
def get_print_formats():
    ...
    return {
        "cash_remittance": settings.cash_remittance,
        "cashier_report": settings.cashier_report,
    }
...
def populate_cr_payment(shift_id):
    ...
    reservation_list = frappe.get_all(
        "Inn Reservation",
        filters={"status": ["in", ["Reserved", "In House", "Finish", "Cancel"]]},
        fields=["*"],
    )
        ...
        if last_shift is None:
            ...
            for reservation_item in reservation_list:
                folio_name = frappe.db.get_value(
                    "Inn Folio", {"reservation_id": reservation_item.name}, ["name"]
                )
                folio_transaction_list = frappe.get_all(
                ...
                for folio_trx_item in folio_transaction_list:
                    ...
                    payment_transaction_doc = frappe.new_doc(
                        "Inn CR Payment Transaction"
                    )
            ...
            master_desk_folio_list = frappe.get_all(
                "Inn Folio",
                filters={
                    "type": ["in", ["Master", "Desk"]],
    ...
    for mode_of_payment_item in mode_of_payment:
        ...
        for cr_payment_detail_item in cr_payment_detail_list:
            if (
                cr_payment_detail_item.mode_of_payment
                == new_payment_detail.mode_of_payment
            ):
...
def populate_cr_refund(shift_id):
    ...
    reservation_list = frappe.get_all(
        "Inn Reservation",
        filters={"status": ["in", ["In House", "Finish", "Cancel"]]},
        fields=["*"],
    )
