...
frappe.ui.form.on("Inn Shift", {
  ...
  onload: function (frm) {
    ...
    if (frm.doc.__islocal === 1) {
      ...
      frappe.call({
        method:
          "inn.inn_hotels.doctype.inn_shift.inn_shift.is_there_open_shift",
        callback: (r) => {
          if (r.message === 1) {
            frappe.set_route("List", "Inn Shift");
            frappe.msgprint(
              __(
                "A Shift already Opened. Please close it first before creating new one."
              )
            );
          } else {
            let cash_count = [1, 5, 10, 20, 50];
  ...
  print_cash_remittance_button: function (frm, cdt, cdn) {
    if (frm.doc.__unsaved) {
      frappe.msgprint(
        __("Please save the document changes first, before printing report")
      );
    } else {
      console.log(cdn);
      print_format_name = "";
      ...
      frappe.call({
        method: "inn.inn_hotels.doctype.inn_shift.inn_shift.get_print_formats",
        async: false,
        callback: (r) => {
          if (r.message) {
            console.log(r.message);
            print_format_name = r.message.cash_remittance;
          } else {
            frappe.msgprint(
              __(
                "Error getting Cashier Report Print Format. Please define it in Inn Hotels Settings."
              )
            );
          }
        },
  ...
  print_cashier_report_button: function (frm, cdt, cdn) {
    if (frm.doc.__unsaved) {
      ...
      frappe.call({
        ...
        callback: (r) => {
          if (r.message) {
            print_format_name = r.message.cashier_report;
          } else {
            frappe.msgprint(
              __(
                "Error getting Cashier Report Print Format. Please define it in Inn Hotels Settings."
      ...
      callback: (r) => {
        if (r.message) {
          ...
          // Opening is over the max opening allowed
            ...
            frappe.msgprint(
              __(
                "Maximum Opening Cash Allowed is below " +
                  format_currency(r.message, "IDR")
              )
            );
          ...
          frappe.msgprint(
            __(
              "Error getting Max Opening Cash Value in Inn Hotels Setting. Please define it First."
            )
          );
        }
      },
  ...
  close_shift_button: function (frm) {
    if (frm.is_dirty()) {
      frappe.msgprint(
        __(
          "There are changes not yet saved in this Shift. Please save the shift first."
        )
      );
    } else {
      frappe.confirm(
        __("You are about to close the shift. Are you sure?"),
        function () {
          frappe.call({
    ...
    callback: (r_payment) => {
      frappe.call({
        ...
        callback: (r_refund) => {
          ...
          frm.set_value(
            "balance",
            frm.doc.opening + total_payment - total_refund
          );
          frm.set_value(
            "cash_balance",
            frm.doc.opening + total_cash_payment - total_refund
          );
        },
      });
    },
