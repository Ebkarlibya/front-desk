...
class ARCityLedgerInvoice(Document):
    ...
    def validate_unique_folios_in_table(self):
        ...
        for item in self.folio:
            ...
            if item.folio_id in seen_folios:
                frappe.throw(
                    _(
                        "Folio '{0}' is duplicated in the Folio to be Collected table. Please ensure each folio is added only once."
                    ).format(item.folio_id),
    ...
    def validate_folios_not_in_other_invoices(self):
        ...
        for item in self.folio:
            ...
            try:
                ...
                frappe.throw(
                    _(
                        "An unexpected error occurred during folio validation for {0}: {1}"
                    ).format(item.folio_id, str(e))
                )
            ...
            if conflicting_invoice_name:
                ...
                frappe.throw(
                    _(
                        "Folio '{0}' is already present in another AR City Ledger Invoice '{1}'. Please remove it from this document or cancel the conflicting invoice."
                    ).format(item.folio_id, conflicting_invoice_name),
    ...
    def validate_payment_totals_not_exceed(self):
        ...
        if flt(total_paid) > flt(total_amount):
            frappe.throw(
                _(
                    "Total payments ({0}) exceed Total amount ({1}). Please correct payments so Outstanding is not negative."
                ).format(total_paid, total_amount),
...
def make_payment(id):
    ...
    unpaid_payments = list(
        filter(lambda p: flt(p.get("paid")) == 0, doc.get("payments"))
    )
    ...
    for payment_row in unpaid_payments:
        remark = _("AR City Ledger Invoice Payments: {0}").format(
            cstr(payment_row.name)
        )
        ...
        try:
            ...
            frappe.db.rollback()
            frappe.log_error(
                f"Failed to create/submit Journal Entry for payment {payment_row.name}. Error: {str(e)}",
                "ACL_Payment_Error",
            )
            frappe.throw(
                _("Failed to process payment {0}: {1}").format(payment_row.name, str(e))
            )
            return 0
...
def cancel_ar_city_ledger_invoice(arci_id, jv_id):
    ...
    try:
        ...
        frappe.log_error(
            f"Error cancelling AR City Ledger Invoice {arci_id} or JE {jv_id}: {str(e)}",
            "ACL_Cancel_Error",
        )
        return False
...
def get_payment_entry_remaining(payment_entry_id, current_arci=""):
    ...
    try:
        ...
        return {
            "error": _("Error loading Payment Entry {0}: {1}").format(
                payment_entry_id, cstr(e)
            )
        }
    ...
    if getattr(pe_doc, "docstatus", 0) != 1:
        return {
            "error": _("Payment Entry {0} is not submitted.").format(payment_entry_id)
        }

    # must be Receive type
    if (getattr(pe_doc, "payment_type", "") or "").lower() != "receive":
        return {
            "error": _("Payment Entry {0} is not of type 'Receive'.").format(
                payment_entry_id
            )
        }
    ...
    try:
        if current_arci:
            ...
            total_allocated = flt(
                frappe.db.sql(sql, (payment_entry_id, current_arci))[0][0]
            )
        else:
            sql = """SELECT COALESCE(SUM(payment_amount), 0) 
                     FROM `tabAR City Ledger Invoice Payment Entry`
        ...
        return {
            "error": _(
                "Failed to compute allocated amount for Payment Entry {0}: {1}"
            ).format(payment_entry_id, cstr(e))
        }
    ...
    for parent_name, child_rows in parents.items():
        try:
            ...
            remaining_rows = [
                ...
                if r.payment_entry_id != payment_entry_id
            ]
            ...
            frappe.log_error(
                f"Error while removing PE links from ARCI {parent_name} for PE {payment_entry_id}: {str(e)}",
                "ARCI_Remove_PE_Link_Error",
            )
...
def make_journal_entry__discount(arci_name):
    ...
    if not arci.name:
        frappe.throw(
            _(
                "Please save AR City Ledger Invoice before creating discount Journal Entry."
            )
        )
    ...
    discount_rows_to_use = [
        r
        for r in discounts
        if not r.get("journal_entry_id") and flt(r.get("payment_amount")) > 0
    ]
    ...
    if total_discount > outstanding:
        frappe.throw(
            _("Total discount ({0}) exceeds the invoice outstanding ({1}).").format(
                total_discount, outstanding
            )
        )
    ...
    if not party_account:
        party_account = frappe.db.get_value(
            "Party Account", {"parent": customer}, "account"
        )
    if not party_account:
        frappe.throw(
            _(
                "No Party Account found for Customer {0}. Please set Party Account under the Customer."
            ).format(customer)
        )
    ...
    try:
        ...
        frappe.log_error(
            message=cstr(e), title="Discount Journal Entry Creation Failed"
        )
        frappe.throw(
            _("Failed to create/submit Journal Entry for discount: {0}").format(cstr(e))
        )
...
def on_journal_entry_cancel_discount(doc, method):
    ...
    for parent_name, rows in parents.items():
        try:
            ...
            frappe.log_error(
                _(
                    "AR City Ledger Invoice {0} not found while cancelling JE {1}"
                ).format(parent_name, je_name)
            )
            continue
