...
def calculate_total_amount(doc, method):
    final_room_rate_only_amount = (
        doc.final_total_rate_amount - doc.final_breakfast_rate_amount
    )
    _, room_rate_before, _, _, bf_rate_before, _ = (
        calculate_rate_breakdown_from_final_rate(
            doc.room_rate_tax,
            doc.breakfast_tax,
            final_room_rate_only_amount,
            doc.final_breakfast_rate_amount,
        )
    )
...
def get_room_rate_after_tax(room_rate_id):
    ...
    _, _, room_rate_after_tax = calculate_inn_tax_and_charges(
        float(doc.room_rate), doc.room_rate_tax
    )
...
def get_breakfast_rate_after_tax(room_rate_id):
    doc = frappe.get_doc("Inn Room Rate", room_rate_id)
    _, _, breakfast_rate_after_tax = calculate_inn_tax_and_charges(
        float(doc.breakfast_rate), doc.breakfast_tax
    )
...
def get_actual_room_rate_before_from_actual_rate(room_rate_id, actual_rate):
    ...
    room_rate_tax_breakdown_list = frappe.get_all(
        "Inn Tax Breakdown",
        filters={
            "parent": frappe.db.get_value(
                "Inn Room Rate", {"name": room_rate_id}, ["room_rate_tax"]
            )
        },
        order_by="idx desc",
        fields=["*"],
    ...
    if len(room_rate_tax_breakdown_list) > 0:
        ...
        for index, item in enumerate(room_rate_tax_breakdown_list):
            ...
            if item.breakdown_type == "On Net Total":
                ...
                if index == 0:
                    tb_rate_before[index] = math.floor(
                        float(actual_room_rate) / denominator
                    )
                    tb_rate_after[index] = actual_room_rate
                else:
                    tb_rate_before[index] = math.floor(
                        float(tb_rate_before[index - 1]) / denominator
                    )
                ...
                if index < len(room_rate_tax_breakdown_list):
                    if index == 0:
                        tb_rate_after[
                            len(room_rate_tax_breakdown_list)
                            - int(item.breakdown_row_id)
                            - 1
                        ] = math.floor(float(actual_room_rate) / denominator)
                        ...
                        tb_rate_after[
                            ...
                            - int(item.breakdown_row_id)
                            - 1
                        ] = math.floor(float(tb_rate_after[index - 1]) / denominator)
                        tb_rate_before[index] = (
                            float(tb_rate_after[index - 1]) / denominator
                        )
...
@frappe.whitelist()
def get_actual_room_rate_breakdown_check_commission(
    room_rate_id, actual_rate: int, reservation_id
):
    ...
    if channel.profit_sharing == PROFIT_SHARING_ENABLED:
        ...
        _, _, room_price = calculate_inn_tax_and_charges_exclude_commision(
            actual_rate - room_rate_doc.final_breakfast_rate_amount,
            room_rate_doc.room_rate_tax,
            channel.room_cashback,
        )
        print(room_price)
...
@frappe.whitelist()
def calculate_rate_breakdown_from_final_rate(
    room_rate_tax_name,
    breakfast_tax_name,
    final_room_rate_amount,
    ...
    if len(room_rate_tax_breakdown_list) > 0:
        ...
        for index, item in enumerate(room_rate_tax_breakdown_list):
            ...
            if item.breakdown_type == "On Net Total":
                denominator = (100 + float(item.breakdown_rate)) / 100
                if index == 0:
                    room_rate_before[index] = math.floor(
                        float(final_room_rate_amount) / denominator
                    )
                    room_rate_after[index] = int(final_room_rate_amount)
                else:
                    room_rate_before[index] = math.floor(
                        float(room_rate_before[index - 1]) / denominator
                    )
        ...
        frappe.msgprint(
            _(
                "Tax Breakdown in "
                + room_rate_tax_name
                + " are not defined. Please define it first."
            )
        )

    # Calculate for breakfast rate
    breakfast_rate_tax_breakdown_list = frappe.get_all(
        "Inn Tax Breakdown",
    ...
    if len(breakfast_rate_tax_breakdown_list) > 0:
        ...
        for index, item in enumerate(breakfast_rate_tax_breakdown_list):
            ...
            if item.breakdown_type == "On Net Total":
                denominator = (100 + float(item.breakdown_rate)) / 100
                if index == 0:
                    bf_rate_before[index] = math.floor(
                        float(final_breakfast_rate_amount) / denominator
                    )
                    bf_rate_after[index] = int(final_breakfast_rate_amount)
                else:
                    bf_rate_before[index] = math.floor(
                        float(bf_rate_before[index - 1]) / denominator
                    )
